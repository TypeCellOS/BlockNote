...
Math blocks require the `@react-pdf/math` package. Invalid LaTeX or Mermaid sources render an error placeholder with the error message, mirroring the editor. Note that the PDF exporter renders LaTeX with MathJax while the editor (and the DOCX/ODT exporters) use KaTeX — rare constructs supported by only one of the two may render differently.

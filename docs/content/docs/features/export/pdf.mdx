...
This works out of the box: the export matches the editor's look (Inter body
text, Geist Mono code) and handles math blocks and emoji — the default fonts
ship inside the package and load lazily on the first export, with no CDN
involved. The Typst compiler itself (a ~30MB wasm file) is the one thing
loaded from a CDN by default; for production you'll want to bundle it: see
[Fonts & offline use](#fonts--offline-use).
...
[example](/examples/interoperability/converting-blocks-to-pdf-ua) bundles
everything explicitly and works fully offline.

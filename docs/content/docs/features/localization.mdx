...
The optional `locale` field selects the localized emoji names, categories,
skin-tone names, and search terms used by the emoji picker. BlockNote's bundled
dictionaries already set this field. When creating a custom dictionary, set it
to the matching locale code; if it is omitted or unsupported, the emoji data
falls back to English.

```tsx
const customFrenchDictionary = {
  ...fr,
  locale: "fr",
  // Your translated or customized dictionary fields.
};
```

The picker interface strings use the same dictionary as the rest of BlockNote:
...
- Set `locale` to the matching locale code so the emoji picker uses the
  appropriate localized data

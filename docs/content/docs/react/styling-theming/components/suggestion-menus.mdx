...
The Slash Menu is a Suggestion Menu that opens with the `/` character (or when clicking the `+` button in the [Block Side Menu](/docs/react/styling-theming/components/side-menu).
...
Changing the items in the new Suggestion Menu, or the component used to render it, is done the same way as for the [Slash Menu](/docs/react/styling-theming/components/suggestion-menus#slash-menu). For more information about how the mentions elements work, see [Custom Inline Content](/docs/reference/editor/custom-schemas/custom-inline-content).

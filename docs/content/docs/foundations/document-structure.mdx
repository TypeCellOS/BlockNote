...
- **`type`**: The block's type, such as a paragraph, heading, or list item. For an overview of built-in block types, see [Default Blocks](/docs/foundations/schemas#default-blocks).

- **`props`**: The block's properties, which is a set of key/value pairs that further specify how the block looks and behaves. Different block types have different props - see [Default Blocks](/docs/foundations/schemas#default-blocks) for more.
...
While most blocks use an array of `InlineContent` objects to describe their content (e.g.: paragraphs, headings, list items), some blocks, like [images](/docs/foundations/schemas#image), don't contain any rich text content, so their `content` fields will be `undefined`.

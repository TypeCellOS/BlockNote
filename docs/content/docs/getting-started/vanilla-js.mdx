...
Make sure to call `editor.registerPortalElement(container)`/`editor.unregisterPortalElement(container)` when each element is added/removed to/from the DOM. This ensures that `editor.isWithinEditor` works correctly.

...
**options:** Configure the editor with options like (see [Editor Options](/docs/reference/editor/options) for all options):

- `initialContent` - Set starting content
- `domAttributes` - Add HTML attributes to editor elements. See [Adding DOM Attributes](/docs/theming#adding-dom-attributes) for more.
- `dictionary` - Customize text strings for localization. See the [Localization](/docs/features/localization) for more.
- `schema` - Add custom blocks and styles. See [Custom Schemas](/docs/custom-schemas).

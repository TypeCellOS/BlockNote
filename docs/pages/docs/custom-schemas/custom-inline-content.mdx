...
Use the `createReactInlineContentSpec` function to create a custom block type. This function takes two arguments:
...
The Inline Content Config describes the shape of the Inline Content, it's type and the properties it supports:
...
The `PropSchema` specifies the props that the inline content supports. The props of inline content is data that is associated with your Inline Content, and can be used to customize the inline content's appearance or behavior.
...
The Inline Content Implementation defines how the block should be rendered in the editor, and how it should be parsed from and converted to HTML.
...
`contentRef:` A React `ref` you can use to mark which element in your block is editable, this is only available if your block config contains `content: "styled"`.

_Note that since inline content is, by definition, inline, your component should also return a HTML inline element._

...
When you highlight content using the mouse or keyboard, this is called a selection. A selection is only active when the highlighted content spans at least 2 blocks. BlockNote uses `Selection` objects to show which blocks the current selection spans across:
...
The demo below displays the blocks in the selection as JSON below the editor. If a selection isn't active, it displays the block containing the [text cursor]() instead.

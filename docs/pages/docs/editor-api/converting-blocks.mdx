...
```typescript
async blocksToMarkdownLossy(blocks?: Block[]): string;
...
```typescript
async tryParseMarkdownToBlocks(markdown: string): Blocks[];
...
```typescript
async blocksToFullHTML(blocks?: Block[]): string;
...
```typescript
async blocksToHTMLLossy(blocks?: Block[]): string;

...
**`filter`**: Filter the comments in the sidebar. Can pass either `"open"` or `"resolved"` to only show open or resolved comments. Shows all comments if undefined.

**`sort`**: Sort the comments in the sidebar. Can pass `"position"`, `"newest"`, or `"oldest"`. Sorting by `"newest"` uses the most recently added comment to sort threads, while `"oldest"` uses the thread creation date. Sorting by `"position"` puts comments in the same order as their reference text in the editor. Defaults to `"position"`.

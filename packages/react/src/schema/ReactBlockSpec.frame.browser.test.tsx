...
const schema = BlockNoteSchema.create().extend({
  blockSpecs: {
    vanilla: createBlockSpec(
      { type: "vanilla", propSchema: {}, content: "inline" },
      {
        render() {
...
it("keeps native editing and selection working across frame changes", async () => {
  ...
  try {
    ...
    await vi.waitFor(() =>
      expect(host.querySelector(".frame-body")?.textContent).toBe("TitleBody"),
    );
...
it("keeps native typing and selection while vanilla frames refresh", async () => {
  const host = document.createElement("div");
  document.body.append(host);
  const editor = BlockNoteEditor.create({
    schema,

...
> = {
    ...
    audio: (block) => {
        // Audio icon SVG
        const icon = (
            <svg height="18" width="18" viewBox="0 0 24 24" fill="#4F8A10" style={{ display: 'inline', verticalAlign: 'middle' }}>
        ...
        const previewWidth = 'previewWidth' in block.props ? (block.props as any).previewWidth : undefined;
        return (
            <div style={{ margin: '8px 0' }}>
                <FileLink url={block.props.url} name={block.props.name} defaultText="Open audio file" icon={icon} />
    ...
    video: (block) => {
        // Video icon SVG
        const icon = (
            <svg height="18" width="18" viewBox="0 0 24 24" fill="#1976D2" style={{ display: 'inline', verticalAlign: 'middle' }}>
        ...
        return (
            <div style={{ margin: '8px 0' }}>
                <FileLink url={block.props.url} name={block.props.name} defaultText="Open video file" icon={icon} />
                <Caption caption={block.props.caption} width={previewWidth} />
            </div>
        );
    },
    file: (block) => {
        // File icon SVG
        const icon = (
            <svg height="18" width="18" viewBox="0 0 24 24" fill="#888" style={{ display: 'inline', verticalAlign: 'middle' }}>
        ...
        return (
            <div style={{ margin: '8px 0' }}>
                <FileLink url={block.props.url} name={block.props.name} defaultText="Open file" icon={icon} />
                <Caption caption={block.props.caption} width={previewWidth} />
            </div>
        );
    },
...
function FileLink({ url, name, defaultText, icon }: { url?: string, name?: string, defaultText: string, icon: React.ReactElement }) {
    return (
        <Link href={url} style={{ textDecoration: 'none', color: '#333', display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 16 }}>
            {icon}
...
function Caption({ caption, width }: { caption?: string, width?: number }) {
    ...
    return (
        <Text style={{ width, fontSize: 13, color: '#888', margin: '4px 0 0 0' }}>{caption}</Text>
    );
}
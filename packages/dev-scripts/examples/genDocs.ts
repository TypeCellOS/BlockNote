 ...
 */
async function generateExampleGroups(projects: Project[]) {
  const target = path.join(COMPONENT_DIR, "exampleGroups.gen.ts");
...
>;
export const exampleGroups: ExampleGroups = ${JSON.stringify(items, undefined, 2)};
`;
...
await generateExampleGroups(projects);

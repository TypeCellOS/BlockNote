...
/**
 * Generates the exampleCodeBlockTabs.gen.ts file that lets us get the relevant
 * files and their content for each example.
 */
async function generateExampleCodeBlockTabs(projects: Project[]) {
  const target = path.join(COMPONENT_DIR, "exampleCodeBlockTabs.gen.ts");
...
export const EXAMPLES_CODE_BLOCK_TABS = ${JSON.stringify(items, undefined, 2)};`;
...
await generateExampleCodeBlockTabs(projects);
